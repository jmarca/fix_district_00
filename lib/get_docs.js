...
function get_it(db,query,cb){
    ...
    viewer(query,function(e,r){
        if(e || !r || r.rows===undefined){
            console.log(e)
            // short term fix, tick to the next doc in couchdb?
            delete query.include_docs
            return viewer(query,function(e,r){
                       if(e) throw new Error('I give up')
                       var nextdoc = r.rows[1].key
                       return couch_set({'db':statedb
                                        ,'doc':db
                                        ,'state':'fetched'
                                        ,value:nextdoc}
                                       ,function(e,r){
                                            throw new Error('trouble with '+query.startkey+' ticked to next '+nextdoc)
                                        })
                   })
        }
        ...
        if(r.rows ===undefined){
            ...
            throw new Error('empty response from all_docs')
        }
        var rows = r.rows
